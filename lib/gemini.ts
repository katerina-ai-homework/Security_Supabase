...
/**
 * Очищает текст от всех вхождений "TL;DR:", маркеров и подобных меток
 */
function cleanText(text: string): string {
  return text
    // Удаляем TL;DR и вариации
    .replace(/TL;DR:?/gi, '')
    .replace(/TLDR:?/gi, '')
    .replace(/T L; D R:?/gi, '')
    .replace(/T L D R:?/gi, '')
    ...
    .replace(/^[-*•·]\s*/gm, '')
    // Удаляем звездочки вокруг текста (например, **текст**)
    .replace(/\*\*([^*]+)\*\*/g, '$1')
    .replace(/\*([^*]+)\*/g, '$1')
...
2. Структура:
   - Заголовок секции (например: "Основные тезисы", "Ключевые моменты")
   - Тезисы (каждый с новой строки)
3. Стиль: Информативный, без воды.
4. Форматирование:
   - Заголовки секций: "## Название секции"
   - Тезисы: просто текст, БЕЗ маркеров, БЕЗ звездочек, БЕЗ дефисов в начале
   - Каждый тезис с новой строки

Пример правильного формата:
## Кратко
Это видео рассказывает о преимуществах использования TypeScript в современных проектах.

## Основные тезисы
TypeScript обеспечивает статическую типизацию, что помогает находить ошибки на этапе компиляции.
Использование интерфейсов улучшает читаемость и поддерживаемость кода.
Интеграция с IDE предоставляет автодополнение и навигацию по коду.
